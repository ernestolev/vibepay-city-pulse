  ...
  return (
    <>
      <motion.button
        ...
        className="relative block w-full text-left focus:outline-none focus-visible:ring-2 focus-visible:ring-offset-2 focus-visible:ring-primary rounded-3xl"
        aria-label={`Open redemption QR for ${offer.title} at ${offer.merchant}`}
      >
        <div
          className={`vibe-glass relative overflow-hidden rounded-3xl p-6 text-[color:var(--vibe-text)] ${
            isPulse ? "vibe-pulse" : ""
          }`}
        >
          ...
          <div className="relative z-10 flex items-center justify-between">
            ...
            <div className="inline-flex items-center gap-1 rounded-full bg-white/15 px-3 py-1 text-[10px] font-semibold uppercase tracking-wider backdrop-blur">
              <Sparkles className="h-3 w-3" />
              Vibe Card
            </div>
          </div>

          <p className="relative z-10 mt-5 text-xs font-semibold uppercase tracking-wider text-[color:var(--vibe-muted)]">
            {offer.brand}
          </p>

          <div className="relative z-10 mt-1 flex items-start justify-between gap-4">
            <div>
              <h3 className="text-3xl font-bold leading-tight tracking-tight text-balance">
                {offer.title}
              </h3>
              <p className="mt-1 text-sm text-[color:var(--vibe-muted)]">at {offer.merchant}</p>
            </div>
          ...
          <div className="relative z-10 mt-5 flex items-center justify-between rounded-2xl bg-white px-5 py-3.5 text-sm font-semibold text-[color:var(--vibe-bg-to)] shadow-lg shadow-black/10">
            <span>Tap to show QR · Redeem now</span>
            <ChevronRight className="h-4 w-4" />
          </div>
        </div>
      </motion.button>

      <QrModal open={open} onOpenChange={setOpen} offer={offer} />
    </>
  );
}
